-- Migration V5: Client Task Review Workflow (Simplified Single-Run Version)
-- Adds direct client linking to tasks and client review status

-- ============================================
-- 1. Add 'client_review' status to task_status enum
-- ============================================
DO $$ 
BEGIN
    -- Add enum value if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'client_review' AND enumtypid = 'task_status'::regtype) THEN
        ALTER TYPE task_status ADD VALUE 'client_review';
    END IF;
END $$;

-- ============================================
-- 2. Add client_id column to tasks table
-- ============================================
ALTER TABLE tasks 
