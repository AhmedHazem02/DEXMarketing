...
import {
    Camera, Clock, Calendar, CheckCircle, Loader2,
...
export default function PhotographerDashboard() {
    ...
    const activeTasks = useMemo(() =>
        tasks?.filter(t => t.department === 'photography' && t.workflow_stage === 'shooting') ?? [],
        [tasks]
    )
    ...
    const handleMarkDone = async (taskId: string) => {
        await markComplete.mutateAsync({ taskId, currentStage: 'shooting' as WorkflowStage })
        ...
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">
                    {isAr ? 'مهام التصوير الفوتوغرافي' : 'Photography Tasks'}
                </h1>
                <p className="text-muted-foreground mt-1">
                    {isAr ? 'المهام المسندة إليك للتصوير' : 'Photo shoot tasks assigned to you'}
                </p>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">{isAr ? 'مهام نشطة' : 'Active'}</CardTitle>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">{isAr ? 'مكتملة' : 'Done'}</CardTitle>
            ...
            <div className="space-y-4">
                <h2 className="text-xl font-semibold">{isAr ? 'جلسات التصوير' : 'Photo Sessions'}</h2>
                {isLoading ? (
                    ...
                    <Card className="p-8 text-center">
                        ...
                        <p className="text-muted-foreground">{isAr ? 'لا توجد جلسات تصوير الآن' : 'No photo sessions scheduled'}</p>
                    </Card>
                ) : (
