...
export function ServiceCard({ item, index, isAr, gradient, accent, span, IconComponent }: ServiceCardProps) {
    ...
    return (
        <motion.div
            ...
            className={`group relative overflow-hidden rounded-3xl glass glass-hover transition-colors duration-500 ${span}`}
        >
            <GlareHover
                glareColor="#ffffff"
                glareOpacity={0.15}
...
export function FallbackServiceCard({ service, isAr, index }: FallbackServiceCardProps) {
    ...
    return (
        <motion.div
            ...
            transition={{ delay: index * 0.08, duration: 0.5 }}
            whileHover={prefersReducedMotion ? undefined : { y: -6, scale: 1.02 }}
            className={`group relative overflow-hidden rounded-3xl glass glass-hover transition-colors duration-500 ${service.span}`}
        >
            <GlareHover
                glareColor="#ffffff"
                glareOpacity={0.15}
